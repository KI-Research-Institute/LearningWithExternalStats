...
load(file=system.file('data/internalTest.RData', package = "LearningWithExternalStats"))
load(file=system.file('data/external.RData', package = "LearningWithExternalStats"))  # External data
load(file=file.path(system.file('data', package = "LearningWithExternalStats"), 'model1.RData')) # model
ls()
```
...
internalResults <- estimateInternalPerformance(dIntTest[['Y']], pInternal, 2)  # TODO change nboot
...
```{r}
internalData <- list(z=dBalanceInt, p = pInternal, y = dIntTest[['Y']])
