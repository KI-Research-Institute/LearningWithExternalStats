...
pInternal <- predict(model1, sapply(d$internalTest[xFeatures], as.numeric), type = "response", s = "lambda.1se")[,1]
internalResults <- estimateInternalPerformance(d$internalTest[['Y']], pInternal, 2)  # TODO change nboot
cat(glue('\nInternal AUC = {format(internalResults$AUC, digits=3)}'), '\n')
```
...
```{r}
dBalanceInt <- computeTable1LikeTransformation(d$internalTest, outcomeBalance=TRUE)
dBalanceExt <- computeTable1LikeTransformation(d$externalTest, outcomeBalance=TRUE)
...
```{r}
internalData <- list(z=dBalanceInt, p = pInternal, y = d$internalTest[['Y']])
estimatedResults <- estimateExternalPerformanceFromStats(
  internalData, muExt, divergence = divergence, lambda = lambda, minW = minW,
  optimizationMethod = optimizationMethod, nboot = 0)
...
```{r}
pExternal <- predict(model1, sapply(d$externalTest[xFeatures], as.numeric), type = "response", s = "lambda.1se")[,1]
extAuc <- auc(roc(d$externalTest[['Y']], pExternal, quiet = TRUE, direction='<'))
cat(glue('\nExternal AUC = {format(extAuc, digits=3)}'), '\n')
```

A more detailed view of estimation results:
